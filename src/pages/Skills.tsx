...
const IN_DEMAND_SKILLS = ["TypeScript", "React", "Node.js", "AWS", "Docker", "Kubernetes", "GraphQL", "Python", "Machine Learning", "CI/CD", "MongoDB", "Redis", "WebSockets", "Tailwind CSS", "Next.js"];
const Skills = () => {
  ...
  const skillsYouHave = IN_DEMAND_SKILLS.filter(skill => userSkillsLower.includes(skill.toLowerCase()));
  const skillsYouNeed = IN_DEMAND_SKILLS.filter(skill => !userSkillsLower.includes(skill.toLowerCase()));
  ...
  return <div className="min-h-screen pt-24 pb-16">
      <div className="container mx-auto px-4 max-w-5xl">
        ...
        <div className="grid md:grid-cols-3 gap-6 mb-10">
          ...
          animationDelay: "200ms"
            ...
            <div className="flex items-center gap-3">
              <p className="text-2xl font-bold text-gradient text-primary-foreground">{matchPercentage}%</p>
        ...
        {/* Your Skills */}
        <div className="glass rounded-xl p-6 mb-10 animate-fade-in-up" style={{
        animationDelay: "300ms"
      }}>
          <h2 className="text-xl font-bold mb-4">Your Skills</h2>
          <div className="flex flex-wrap gap-2">
            {resumeData.skills.map(skill => <span key={skill} className="px-3 py-1.5 bg-success/20 text-success rounded-full text-sm font-medium border border-success/30">
                {skill}
              </span>)}
          </div>
        </div>
        ...
        animationDelay: "400ms"
          ...
          <div className="glass rounded-xl p-6">
            ...
            <div className="flex flex-wrap gap-2">
              {skillsYouHave.length > 0 ? skillsYouHave.map(skill => <span key={skill} className="px-3 py-1.5 bg-success/20 text-success rounded-full text-sm font-medium border border-success/30">
                    {skill}
                  </span>) : <p className="text-muted-foreground">No matching in-demand skills found</p>}
            </div>
          </div>
          ...
          <div className="glass rounded-xl p-6">
            ...
            <div className="flex flex-wrap gap-2">
              {skillsYouNeed.map(skill => <Tooltip key={skill}>
                  <TooltipTrigger>
